...
has _compression_types => sub {
    [map { Scalar::Util::reftype($_) ? $_ : {ext => $_, encoding => $_} } @{shift->compression_types}]
};

has should_serve_asset => sub { sub { $_->path !~ /\.(pdf|jpe?g|gif|png|webp)$/i } };
...
before serve_asset => sub {
    ...
    my ($compressed_asset, $compression_type) = $self->_get_compressed_asset_and_compression_type($req_headers->accept_encoding, $asset, $compression_possibilities);
    ...
    if ($match and $match =~ /-(.*)"$/) {
        if (my @compression_types = grep { $_->{encoding} eq $1} @{$self->_compression_types}) {
            if (@compression_types > 1) {
                warn "more than one matching compression type found for If-None-Match $match and encoding $1";
                return;
            }
            ...
            warn "Found compression type with encoding of $compression_type->{encoding} in If-None-Match $match, but asset at $compressed_asset_path does not exist";

            # return and let Mojolicious::Static::is_fresh fail on etag and the uncompressed asset can be served
            return;
        }
    }

    for my $compression_type (@$compression_possibilities) {
        next unless my $compressed_asset = $self->_get_compressed_file($asset->path . '.' . $compression_type->{ext});
        if ($compressed_asset->size >= $asset->size) {
            warn 'Compressed asset ' . $compressed_asset->path . ' is ' . $compressed_asset->size
                . ' bytes, and uncompressed asset ' . $asset->path . ' is ' . $asset->size
...
L<Mojolicious::Static::Role::Compressed> is a role for L<Mojolicious::Static> that provides the ability to serve pre-compressed versions of static
asset. L<Mojolicious::Static::Role::Compressed> does this by using the before method modifier on L<Mojolicious::Static/serve_asset> and L<Mojolicious::Static/is_fresh>.
...
The asset passed to L<Mojolicious::Static/serve_asset> is a L<Mojo::Asset::File> (L<Mojo::Asset/is_file> returns C<1>).

=item *

It is determined that the asset should be served by L</should_serve_asset> being C<undef> or a subroutine that returns C<1> for the given L<Mojo::Asset::File>.

=item *

L<Mojo::Headers/accept_encoding> for the request contains at least one encoding listed in L</compression_types>.
...
L<Mojolicious::Static::Role::Compressed> uses the same modified time as the original asset when setting L<Mojo::Headers/last_modified> in the response, and modifies the ETag
(L<Mojo::Headers/etag>) in the response by appending C<"-$encoding"> (i.e. "etag-gzip"), where the encoding is specified in L</compression_types>. This is in line with
L<RFC-7232|https://tools.ietf.org/html/rfc7232#section-2.3.3>, which explicitly states that ETags should be content-coding aware.
...
in request and response headers to specify the encoding. Below is an example of this and the default for L</compression_types>:
...
Compression types will be checked for in the order they are specified, with the first one that matches all of the requirements in L</DESCRIPTION> being used.
L</compression_types> should not be changed once L<Mojo::Static> begins serving assets, so if you want to change these when the app is already running, you should create a
new L<Mojolicious::Static> object and add the role and your config again (although I'm not sure why you would want to change this once the app is already running and serving
assets).
...
L</should_serve_asset> is a subroutine that determines whether or not L<Mojolicious::Static::Role::Compressed> should attempt to serve a compressed version of a
L<Mojo::Asset::File>. The subroutine is passed the L<Mojo::Asset::File> in C<$_>. The default is to not look for compressed versions of any assets whose L<Mojo::Asset::File/path>
...
To look for compressed versions of all assets, set L</should_serve_asset> to C<undef>:
...
L<Mojolicious::Static::Role::Compressed> uses the stash keys C<mojolicious_static_role_compressed.asset> and C<mojolicious_static_role_compressed.compression_type> internally,
so these should not be used by elsewhere in the L<Mojolicious> app. There are no plans for other stash keys, but other keys under C<mojolicious_static_role_compressed.*> should
